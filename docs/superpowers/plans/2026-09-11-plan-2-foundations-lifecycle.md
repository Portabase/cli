...
- Ne pas toucher `commands/agent.py`, `commands/db.py`, `commands/dashboard.py`, `commands/decrypt.py`, `core/crypto.py`, `core/network.py`, `core/docker.py`, `templates/compose.py` (réécrits ou supprimés au Plan 4). `decrypt` (ajouté en 26.08.12) est enregistré via `LegacyCommand` comme `agent`/`dashboard`. `core/utils.py` : seulement retirer `current_version` (Task 2).
...
from commands import decrypt as legacy_decrypt
...
def build_app(
    ...
    commands = [
        ...
        LegacyCommand(ui, telemetry, legacy_decrypt.decrypt, name="decrypt", help="Decrypt Portabase .enc backup files (single file or folder).", panel="Configuration"),
...
"commands/decrypt.py" = ["B904", "TID251"]
"core/crypto.py" = ["BLE001", "SIM105"]
...
Expected: panneaux `Creation` (agent, dashboard), `Lifecycle` (start, stop, restart, logs, uninstall), `Configuration` (decrypt, db, config), `System` (update) ; options `--version`, `--verbose`, `--debug`, `--no-color`, `--non-interactive`.
