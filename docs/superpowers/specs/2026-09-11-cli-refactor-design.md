...
| `decrypt INPUT [OUTPUT]` | `--key` | déchiffre des sauvegardes `.enc` (ajouté en 26.08.12) ; `DecryptCommand`, `DecryptionError(PortabaseError)` code `E_CRYPTO` exit 8. `core/crypto.py` reste un module de fonctions pures. |
...
| `DecryptionError` (`core/crypto.py`) | `E_CRYPTO` | 8 |
